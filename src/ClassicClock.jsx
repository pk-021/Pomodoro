...
function CountdownCircle({ total, elapsed, endval, color, radius}) {
    const startAngle = -90;
    const iangle = elapsed / total * 360;
    const fangle = endval/total *360;
    ...
    // Calculate the complement iangle (the portion that's NOT elapsed)
    ...
    // For the complement arc
    const complementStart = polarToCartesian(cx, cy, r, startAngle + iangle);
    const complementEnd = polarToCartesian(cx, cy, r, startAngle + fangle);
    
    const largeArcFlag = complementAngle > 180 ? 1 : 0;
    
    // SVG arc path for the complement (remaining time)
    const pathData = [
      `M ${cx} ${cy}`,
...
function ClassicClock(props) {

    const hourRef = useRef(null);
    const hourShadowRef = useRef(null);
    const minuteRef = useRef(null);
    const minuteShadowRef = useRef(null);
    const secondRef = useRef(null);
    const secondShadowRef = useRef(null);

    console.log(props.totalTime)

    let minutes = Math.floor(props.totalTime / (60 * 1000));
    let seconds = Math.ceil((props.totalTime / 1000) % 60);
    ...
    if (secondRef.current)
        secondRef.current.setAttribute("transform", `rotate(${s * 6}, 256, 256)`);
    if (secondShadowRef.current)
        secondShadowRef.current.setAttribute("transform", `rotate(${s * 6}, 253, 259)`);

    if (minuteRef.current)
        minuteRef.current.setAttribute("transform", `rotate(${m * 6}, 256, 256)`);
    if (minuteShadowRef.current)
        minuteShadowRef.current.setAttribute("transform", `rotate(${m * 6}, 254, 258)`);

    if (hourRef.current)
        hourRef.current.setAttribute("transform", `rotate(${h * 30}, 256, 256)`);
    if (hourShadowRef.current)
        hourShadowRef.current.setAttribute("transform", `rotate(${h * 30}, 255, 257)`);
    ...
    return (
        ...
        C471.2,384.677,498.689,323.55,498.689,256C498.688,121.401,389.573,12.288,254.976,12.288z" />
                <path fill="none" stroke="#000000" strokeWidth="0.5" strokeMiterlimit="10" d="M173.297,437.792l-2.04,4.487 M211.854,455.793
        ...
        C455.424,366.141,366.14,455.424,256,455.424z" />
                ...
                <g id="pies" transform="translate(256,256)">

                    {CountdownCircle({ total: 60, elapsed: m, endval:(m + minutes), radius: 70, color: "rgba(0, 255, 0, 0.5)" })};
                    {CountdownCircle({ total: 60, elapsed:s, endval:(s+seconds), radius: 50, color: "rgba(255,0 , 0, 0.5)"})};
                </g>
        ...
        C266.79,109.068,265.623,110.899,264.889,112.41z" />
            ...
            edited for timer 
                ...
                <path d="M237.59,94.56c1.248-2.662,2.56-4.568,3.936-5.721c1.376-1.153,2.888-1.729,4.537-1.729c2.656,0,4.885,1.438,6.684,4.318
    c1.801,2.881,2.699,6.522,2.699,10.925c0,5.3-1.063,9.703-3.189,13.21s-4.762,5.263-7.896,5.263c-2.208,0-4.176-0.94-5.904-2.822
    c-1.729-1.883-3.14-4.82-4.236-8.813c-1.097-3.993-1.644-8.651-1.644-13.978c0-5.761,0.672-10.981,2.016-15.668
    c1.345-4.685,3.06-8.197,5.148-10.539c2.088-2.345,4.268-3.517,6.542-3.517c2.336,0,4.318,1.024,5.953,3.071
    c1.357,1.664,2.037,3.661,2.037,5.99c0,1.563-0.299,2.842-0.898,3.841c-0.602,0.998-1.332,1.498-2.195,1.498
    ...
    c0.354-0.896,0.529-1.603,0.529-2.112c0-0.716-0.219-1.306-0.648-1.767c-0.574-0.589-1.367-0.884-2.376-0.884
    c-1.552,0-2.967,0.781-4.248,2.343c-1.28,1.563-2.364,4.243-3.252,8.045c-0.888,3.802-1.333,8.07-1.333,12.808
    C237.518,92.641,237.542,93.677,237.59,94.56z M244.742,90.873c-2,0-3.637,1.158-4.908,3.477
    c-1.272,2.316-1.908,5.573-1.908,9.771c0,4.557,0.616,8.04,1.848,10.443c1.232,2.407,2.8,3.608,4.704,3.608
    c1.89,0,3.408-1.138,4.562-3.417c1.15-2.278,1.729-5.685,1.729-10.216c0-4.813-0.532-8.296-1.597-10.444
    C248.108,91.949,246.631,90.873,244.742,90.873z" />


            0 in 60
                <path d="M270.59,65.56c3.12,0,5.72,1.946,7.8,5.837c2.576,4.787,3.864,12.045,3.864,21.773
    c0,9.037-1.121,15.916-3.36,20.64c-2.241,4.723-4.944,7.084-8.112,7.084c-3.056,0-5.608-1.983-7.656-5.953
    c-2.56-4.992-3.84-12.198-3.84-21.619c0-8.96,1.208-16.089,3.624-21.389C264.846,68.676,267.406,65.56,270.59,65.56z
    M270.734,68.325c-2.113,0-3.76,1.62-4.944,4.858c-1.184,3.239-1.776,9.953-1.776,20.141c0,9.959,0.612,16.602,1.836,19.929
    c1.224,3.328,2.868,4.992,4.932,4.992c2.08,0,3.68-1.536,4.801-4.608c1.328-3.661,1.992-10.342,1.992-20.044
    c0-10.035-0.624-16.755-1.872-20.161C274.355,70.128,272.697,68.325,270.734,68.325z" />


                5 in 50
                <path d="M316.336,90.76c4.048,1.484,7.354,2.229,9.913,2.229c1.199,0,2.327-0.188,3.384-0.56c1.058-0.369,1.851-0.799,2.376-1.285
    c0.272-0.229,0.448-0.347,0.528-0.347c0.159,0,0.239,0.192,0.239,0.577c0,0.896-0.36,2.079-1.08,3.553
    c-0.722,1.471-1.7,2.644-2.94,3.514c-1.239,0.871-2.667,1.307-4.285,1.307c-1.872,0-4.104-0.461-6.693-1.383l-0.783,14.67
    c1.216-0.975,2.375-1.703,3.478-2.188c1.104-0.488,2.151-0.73,3.144-0.73c3.136,0,5.77,1.664,7.904,4.992
    c2.136,3.328,3.201,7.437,3.201,12.326c0,5.428-1.135,9.894-3.405,13.4c-2.272,3.507-5.219,5.26-8.832,5.26
    ...
    c-0.96,0-1.92,0.281-2.881,0.844c-0.96,0.564-1.92,1.409-2.88,2.535c-0.111,1.792-0.313,2.989-0.601,3.592
    c-0.287,0.603-0.656,0.902-1.104,0.902c-0.32,0-0.576-0.18-0.768-0.538c-0.193-0.358-0.288-0.856-0.288-1.497
    c0-1.127,0.327-2.24,0.982-3.342c0.192-0.332,0.327-0.627,0.407-0.883l0.559-11.288C316.212,99.285,316.33,94.959,316.336,90.76z" />

                    0 in 50
                <path d="M350.59,90.76c3.12,0,5.72,1.946,7.8,5.837c2.576,4.787,3.864,12.045,3.864,21.773
    c0,9.037-1.121,15.916-3.36,20.64c-2.241,4.723-4.944,7.084-8.112,7.084c-3.056,0-5.608-1.983-7.656-5.953
    c-2.56-4.992-3.84-12.198-3.84-21.619c0-8.96,1.208-16.089,3.624-21.389C344.846,93.676,347.406,90.56,350.59,90.56z
    M350.734,93.325c-2.113,0-3.76,1.62-4.944,4.858c-1.184,3.239-1.776,9.953-1.776,20.141c0,9.959,0.612,16.602,1.836,19.929
    c1.224,3.328,2.868,4.992,4.932,4.992c2.08,0,3.68-1.536,4.801-4.608c1.328-3.661,1.992-10.342,1.992-20.044
    c0-10.035-0.624-16.755-1.872-20.161C354.355,95.128,352.697,93.325,350.734,93.325z" />
 */}


                <g id="Hour" ref={hourRef}>
                    <use xlinkHref="#p1">
                    </use>
                </g>
                <g id="HourShadow" ref={hourShadowRef}>
                    <use filter="url(#f5)" opacity="0.5" x="-1" y="1" xlinkHref="#p1">
                    </use>
                </g>
                <g id="Minute" ref={minuteRef}>
                    <use xlinkHref="#p2">
                    </use>
                </g>
                <g id="MinuteShadow" ref={minuteShadowRef}>
                    <use filter="url(#f5)" opacity="0.5" x="-2" y="2" xlinkHref="#p2">
                    </use>
                </g>
                <g id="Second" ref={secondRef} >
                    <use xlinkHref="#p3">
                    </use>
                </g>
                <g id="SecondShadow" ref={secondShadowRef}>
                    <use filter="url(#f5)" opacity="0.5" x="-3" y="3" xlinkHref="#p3">
                    </use>
                </g>
